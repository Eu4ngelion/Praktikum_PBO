...
public class MenuAdmin {
    ...
    public void addKamar() {
        ...
        inputKamar.setIdKamar(scanner.nextInt());
        scanner.nextLine();
        for (Kamar kamar : daftarKamar) {
            if (kamar.getIdKamar() == inputKamar.getIdKamar()) {
                System.out.println("ID Kamar ("+ kamar.getIdKamar() +") sudah digunakan!");
                scanner.nextLine();
                return;
            }
        }
        System.out.print("Lantai: ");
        inputKamar.setLantai(scanner.nextInt());
        scanner.nextLine();
        System.out.print("Harga: ");
        inputKamar.setHarga(scanner.nextDouble());
        System.out.print("Status (1: Disewakan, 0: Kosong): ");
        inputId = scanner.nextInt();
        scanner.nextLine();
        if (inputId == 1) {
            inputKamar.setIsDisewakan(true);
        } else if (inputId == 0) {
            inputKamar.setIsDisewakan(false);
        } else {
        ...
        daftarKamar.add(new Kamar(inputKamar.getIdKamar(), inputKamar.getLantai(), inputKamar.getHarga(), inputKamar.getIsDisewakan()));
        ...
        for (Kamar kamar : daftarKamar) {
            if (kamar.getIdKamar() == inputId) {
                ...
                for (Kamar k : daftarKamar) {
                    if (k.getIdKamar() != kamar.getIdKamar() && k.getIdKamar() == inputId ) {
                        System.out.println("ID Kamar ("+ k.getIdKamar() +") sudah digunakan!");
                        scanner.nextLine();
                        return;
                    }
                }
                kamar.setIdKamar(inputId);
                System.out.println("Lantai: " + kamar.getLantai());
                System.out.print("Lantai Baru: ");
                kamar.setLantai(scanner.nextInt());
                scanner.nextLine();
                System.out.println("Harga: " + kamar.getHarga());
                System.out.print("Harga Baru: ");
                kamar.setHarga(scanner.nextDouble());
                System.out.println("Status: " + (kamar.getIsDisewakan() ? "Disewakan" : "Kosong"));
                System.out.print("Status Baru (1: Disewakan, 0: Kosong): ");
                inputId = scanner.nextInt();
                if (inputId == 1) {
                    kamar.setIsDisewakan(true);
                } else if (inputId == 0) {
                    kamar.setIsDisewakan(false);
                } else {
                    System.out.println("Status tidak valid!");
                    scanner.nextLine();
                    return;
    ...
    public void deleteKamar(){
        ...
        for (Kamar kamar : daftarKamar) {
            if (kamar.getIdKamar() == inputId) {
                for (Penghuni penghuni : daftarPenghuni) {
                    if (penghuni.idKamar == kamar.getIdKamar()) {
    ...
    public void addPenghuni(){
        ...
        isFound = false;
        for (Kamar kamar : daftarKamar) {
            if (kamar.getIdKamar() == inputPenghuni.idKamar) {
                isFound = true;
                break;
            }
        }
    ...
    public void updatePenghuni(){
        ...
        for (Penghuni penghuni : daftarPenghuni) {
            if (penghuni.idPenghuni == inputId) {
                ...
                isFound = false;
                for (Kamar kamar : daftarKamar) {
                    if (kamar.getIdKamar() == penghuni.idKamar) {
                        isFound = true;
                        break;
                    }
                }
